...
HRESULT BrokerClient::FindAndConnectToBroker()
{
    ...
    if (SUCCEEDED(device->QueryInterface(&dxgiDevice)) && SUCCEEDED(dxgiDevice->GetAdapter(&adapter)) && SUCCEEDED(adapter->GetDesc(&desc))) {
        if (memcmp(&pManifestView->adapterLuid, &desc.AdapterLuid, sizeof(LUID)) == 0) {
            ...
            if (device1 && device5) {
                wil::unique_handle hTexture(VirtuaCam::Utils::Win32::GetHandleFromObjectName(pManifestView->textureName));
                if (hTexture && SUCCEEDED(device1->OpenSharedResource1(hTexture.get(), IID_PPV_ARGS(_producer.sharedTexture.GetAddressOf())))) {
                    wil::unique_handle hFence(VirtuaCam::Utils::Win32::GetHandleFromObjectName(pManifestView->fenceName));
                    if (hFence && SUCCEEDED(device5->OpenSharedFence(hFence.get(), IID_PPV_ARGS(_producer.sharedFence.GetAddressOf())))) {
...
HRESULT BrokerClient::Generate(IMFSample* sample, REFGUID format, IMFSample** outSample)
{
    ...
    *outSample = nullptr;
    ...
    if (_producer.isConnected) {
        ...
        context->OMSetRenderTargets(1, _textureRTV.GetAddressOf(), nullptr);

        context->VSSetShader(_blitVS.Get(), nullptr, 0);
        context->PSSetShader(_blitPS.Get(), nullptr, 0);
        context->PSSetShaderResources(0, 1, _producerSRV.GetAddressOf());
        context->PSSetSamplers(0, 1, _blitSampler.GetAddressOf());
        context->IASetInputLayout(nullptr);
        context->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        context->Draw(3, 0);

    } else {
        const float redColor[] = { 0.8f, 0.1f, 0.0f, 1.0f };
        context->ClearRenderTargetView(_textureRTV.Get(), redColor);
    }
    context->Flush();
