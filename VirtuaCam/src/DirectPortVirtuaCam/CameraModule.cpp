...
static int                          g_currentResolutionIndex = 4; // Default to 720p
static UINT                         g_currentSharedSize = g_sharedResolutions[4];
        ...
        while (WM_QUIT != msg.message) {
            if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
                ...
                if (g_sharedFence) {
                    ...
                    if (g_pManifestView) {
                         InterlockedExchange64(reinterpret_cast<volatile LONGLONG*>(&g_pManifestView->frameValue), g_sharedFrameValue);
                    }
                }
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow) {
    ...
    wcex.hIcon = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(IDI_VIRTUACAM), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
    wcex.hIconSm = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(IDI_VIRTUACAM), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_SHARED);
...
void InternalChangeSharedResolution(BOOL bCycleUp) {
    ...
    else g_currentResolutionIndex = (g_currentResolutionIndex == 0) ? ((int)g_sharedResolutions.size() - 1) : (g_currentResolutionIndex - 1);
