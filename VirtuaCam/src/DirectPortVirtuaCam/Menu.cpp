    ...
    if (m_parentMenu == nullptr) {
        ...
        while (g_topLevelMenu != nullptr) {
            if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
                ...
                for (auto menu : g_openMenus) {
                    if (menu && menu->m_hwnd && IsDialogMessage(menu->m_hwnd, &msg)) {
                        processed = true;
                        break;
                    }
                }
                ...
                // While the menu is active, do not run the background rendering task.
                // Yield CPU time to avoid a 100% busy loop.
                Sleep(10);
            }
        }
        return m_result;
    ...
    case WM_LBUTTONUP: {
        if (m_hoverItem != -1) {
            ...
            if (!item.isSubMenu && item.id != 0) {
                if (g_topLevelMenu) g_topLevelMenu->m_result = item.id;
                CloseAllMenus();
            }
        }
        return 0;
