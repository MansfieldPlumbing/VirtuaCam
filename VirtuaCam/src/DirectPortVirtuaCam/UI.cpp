...
namespace VirtuaCam { class Consumer; }
extern const VirtuaCam::Consumer* GetGlobalConsumer();
...
extern void SetSourceMode(SourceMode newMode, DWORD discoveredPID = 0);
...
void ShowContextMenu(HWND hwnd) {
    ...
    CustomMenu menu(hwnd, g_instance);
    ...
    if (sourcesSubMenu) {
        ...
        sourcesSubMenu->AddItem(L"Camera", ID_SOURCE_CAMERA, currentMode == SourceMode::Camera);
        
        const VirtuaCam::Consumer* consumer = GetGlobalConsumer();
        if (consumer && !consumer->GetDiscoveredStreams().empty()) {
            const auto& streams = consumer->GetDiscoveredStreams();
            bool hasVisibleDiscoveredStream = false;
            for (const auto& stream : streams) {
                if (stream.processId != GetCurrentProcessId()) {
                    hasVisibleDiscoveredStream = true;
                    break;
                }
            }

            if (hasVisibleDiscoveredStream) {
                sourcesSubMenu->AddSeparator();
                for (size_t i = 0; i < streams.size(); ++i) {
                    const auto& stream = streams[i];
                    if (stream.processId == GetCurrentProcessId()) continue;
    ...
    const int menuWidth = menu.GetWidth();
    const int menuHeight = menu.CalculateHeight();
    HMONITOR hMonitor = MonitorFromPoint(pt, MONITOR_DEFAULTTONEAREST);
    MONITORINFO mi = { sizeof(mi) };
    GetMonitorInfo(hMonitor, &mi);
    int x = pt.x;
    int y = pt.y;
    if (x + menuWidth > mi.rcWork.right) { x = mi.rcWork.right - menuWidth; }
    if (x < mi.rcWork.left) { x = mi.rcWork.left; }
    if (y + menuHeight > mi.rcWork.bottom) { y = mi.rcWork.bottom - menuHeight; }
    if (y < mi.rcWork.top) { y = mi.rcWork.top; }
    
    SetForegroundWindow(hwnd);
    UINT result = menu.Show(x, y, g_onIdle);

    if (result == ID_SOURCE_PASSTHROUGH) {
        SetSourceMode(SourceMode::Passthrough);
    } else if (result == ID_SOURCE_CAMERA) {
        SetSourceMode(SourceMode::Camera);
    } else if (result >= ID_SOURCE_DISCOVERED_FIRST) {
        const VirtuaCam::Consumer* consumer = GetGlobalConsumer();
        if (consumer) {
    ...
    } else if (result >= ID_AUDIO_DEVICE_NONE) {
...
LRESULT CALLBACK PreviewWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ...
        }
        {
           ...
           {
               MessageBox(hwnd, L"Broker texture function not available.", L"Error", MB_OK | MB_ICONERROR);
               return -1;
           }
